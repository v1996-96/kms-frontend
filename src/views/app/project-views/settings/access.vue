  ...
  <v-card class="mb-2">
    ...
    <v-data-table
      ...
      <template slot="items" slot-scope="props">
        ...
        <td>
          <v-btn flat small color="primary" v-if="!props.item.system" @click="setRoleEditing(props.item.project_role_id)">Edit</v-btn>
          <v-btn flat small color="info" v-if="props.item.system" @click="setRoleEditing(props.item.project_role_id)">Info</v-btn>
...
export default {
  name: 'kms-role-project-settings-roles-tab',
  mixins: [
    SearchPaginateMixin({
      namespace: 'roles',
  ...
  methods: {
    ...
    updateRole (role) {
      Api.projects.updateRole({
        id: role.project_role_id,
        model: role
      }).catch(info => {
    ...
    deleteRole () {
      ...
      Api.projects.removeRole({
        id: this.deletingRole.project_role_id
      }).then(() => {
